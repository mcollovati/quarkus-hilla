...
public class QuarkusPluginAdapter implements PluginAdapterBuild {
    ...
    public File frontendResourcesDirectory() {
        return resolveProjectDirectory(config.frontendResourcesDirectory(), "vaadin.build.frontendResourcesDirectory");
    }
    ...
    @Override
    public boolean checkRuntimeDependency(
            String groupId, String artifactId, Consumer<String> missingDependencyMessageConsumer) {
    ...
    public File frontendDirectory() {
        return resolveProjectDirectory(config.frontendDirectory(), "vaadin.build.frontedDirectory");
    }

    private File resolveProjectDirectory(File directory, String name) {
        return resolveDirectory(projectBaseDirectory().toFile(), directory, name);
    }

    private File resolveBuildDirectory(File directory, String name) {
        return resolveDirectory(resourcesDir.getOutputDir().toFile(), directory, name);
    }
    ...
    private File resolveDirectory(File base, File directory, String key) {
        ...
        if (directory.exists() && !directory.isDirectory()) {
            throw new ConfigurationException(key + " must be a directory: " + directory, Set.of(key));
        }
        return directory;
    }
    ...
    public File generatedTsFolder() {
        return config.generatedTsFolder().orElseGet(() -> frontendDirectory()
                .toPath()
                .resolve(FrontendUtils.GENERATED)
                .toFile());
    }
    ...
    public URI nodeDownloadRoot() throws URISyntaxException {
        String nodeDownloadRoot =
                config.nodeDownloadRoot().orElseGet(() -> Platform.guess().getNodeDownloadRoot());
        try {
    ...
    public Path projectBaseDirectory() {
        return model.getApplicationModule().getModuleDir().toPath();
    }
    ...
    public File servletResourceOutputDirectory() {
        return resolveBuildDirectory(config.resourceOutputDirectory(), "resourceOutputDirectory");
    }
    ...
    public String buildFolder() {
        Path projectDir = model.getApplicationModule().getModuleDir().toPath();
    ...
    public boolean isReactEnabled() {
        return config.reactEnabled()
                .orElseGet(() -> FrontendUtils.isReactRouterRequired(BuildFrontendUtil.getFrontendDirectory(this)));
    }
    ...
    public static Stream<Path> buildClasspath(ApplicationModel model) {
        return Stream.concat(
                ...
                model.getRuntimeDependencies().stream().flatMap(dep -> dep.getResolvedPaths().stream()));
    }
}
