...
final class InjectableObjectFactoryFieldUsageRemovalMethodVisitor extends MethodNode {
    public InjectableObjectFactoryFieldUsageRemovalMethodVisitor(
            MethodVisitor mv, int access, String name, String descriptor, String signature, String[] exceptions) {
        super(Gizmo.ASM_API_VERSION, access, name, descriptor, signature, exceptions);
        this.mv = mv;
    }
    ...
    public void visitEnd() {
        ...
        while ((fieldUsage = findFieldUsage(iterator)) != null) {
            if (fieldUsage.getOpcode() == Opcodes.PUTFIELD) {
                 ...
                 */
                nodesToReplace.put(iterator.previous(), new InsnNode(Opcodes.NOP)); // PUTFIELD
                nodesToReplace.put(iterator.previous(), new InsnNode(Opcodes.NOP)); // INVOKESTATIC
                nodesToReplace.put(iterator.previous(), new InsnNode(Opcodes.NOP)); // LDC
                nodesToReplace.put(iterator.previous(), new InsnNode(Opcodes.NOP)); // ALOAD
                ...
                nodesToReplace.put(
                        iterator.previous(),
                        new LdcInsnNode(Type.getType("Lorg/atmosphere/inject/Injectable;"))); // ALOAD
                AbstractInsnNode stop = fieldUsage;
                findNextNode(iterator, n -> n == stop);
            }
        }
    ...
    private FieldInsnNode findFieldUsage(ListIterator<AbstractInsnNode> iterator) {
        return (FieldInsnNode) findNextNode(
                iterator, node -> node instanceof FieldInsnNode field && field.name.equals("injectableServiceLoader"));
    }

    private AbstractInsnNode findNextNode(ListIterator<AbstractInsnNode> iterator, Predicate<AbstractInsnNode> test) {
