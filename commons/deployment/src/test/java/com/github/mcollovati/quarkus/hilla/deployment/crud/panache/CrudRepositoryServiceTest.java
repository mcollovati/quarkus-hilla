...
class CrudRepositoryServiceTest {
    ...
    void save_newEntity_entityPersisted() {
        ...
        Assertions.assertThat(newEntity.id).isNotNull();
        Assertions.assertThat(newEntity.isPersistent()).isTrue();
    }
    ...
    void delete_entityDeleted() {
        ...
        allEntities =
                entityManager.createQuery("from TestEntity", TestEntity.class).getResultList();
        Assertions.assertThat(allEntities).hasSize(initialSize - 1).noneMatch(e -> e.id.equals(existingEntity.id));
    }
