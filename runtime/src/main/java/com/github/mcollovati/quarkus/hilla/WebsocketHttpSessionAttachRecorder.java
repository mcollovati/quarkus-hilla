...
public class WebsocketHttpSessionAttachRecorder {
    ...
    public Handler<RoutingContext> createWebSocketHandler(
        ...
        UpgradeRequestSessionAttachmentHandler handler = new UpgradeRequestSessionAttachmentHandler(deployment);
        ...
        return new VertxWebSocketHandler(container.getValue(), info.getValue()) {
            ...
            public void handle(RoutingContext event) {
                ...
                if (HandlerHelper.RequestType.PUSH
                        .getIdentifier()
                        .equals(event.request().getParam(ApplicationConstants.REQUEST_TYPE_PARAMETER))) {
                    super.handle(event);
                } else {
                    event.next();
                }
            ...
            protected VertxWebSocketHttpExchange createHttpExchange(RoutingContext event) {
                ...
                VertxHttpExchange delegate = new VertxHttpExchange(event.request(), null, executor, null, null);
                return handler.extractSession(event, delegate)
                        .<VertxWebSocketHttpExchange>map(
                                sess -> new QuarkusVertxWebSocketHttpExchange(executor, event, sess))
    ...
    private static class UpgradeRequestSessionAttachmentHandler {
        ...
        private void handleRequest(HttpServerExchange exchange) throws Exception {
            ...
            final ServletPathMatch pathMatch =
                    new ServletPathMatches(deployment).getServletHandlerByPath(exchange.getRelativePath());
            ...
            final ServletRequestContext servletRequestContext =
                    new ServletRequestContext(servletContext.getDeployment(), request, response, pathMatch);
        ...
        private Optional<HttpSession> extractSession(RoutingContext event, HttpExchange httpExchange) {
            ...
            HttpServerExchange serverExchange = new HttpServerExchange(httpExchange, -1);
            ...
            ServletContextImpl servletContext = deployment.getServletContext();
            return Optional.ofNullable(sessionManager.getSession(serverExchange, sessionConfig))
                    .map(sess -> HttpSessionImpl.forSession(sess, servletContext, false));
        }
    }
    ...
    private static class QuarkusVertxWebSocketHttpExchange extends VertxWebSocketHttpExchange {
        ...
        public QuarkusVertxWebSocketHttpExchange(
                Executor executor, RoutingContext routingContext, HttpSession session) {
